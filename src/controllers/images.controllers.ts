...
export const turf_visualizer_handler = async (req: Request, res: Response) => {
  try {
    ...
    const [image, userId] = [req.body.image, req.body.userId];
    console.log(image, userId);
    taskTracker.start();
    const output = await replicate.run(
      "leandroamaral/segmentanything:03344e9166342605cdb629b3cc33bf5b479c919245155af222463a5d42fa97d4",
      {
        input: {
          iou: 0.7,
          conf: 0.25,
    ...
    console.log(output);
    taskTracker.stop();
    console.log(`2-Generating mask :${taskTracker.getTime()}`);
